...
3. Write down the value of mean and then return the sample to the population.
...
* **The expected value** of the mean, denoted $E(\bar X)$, is the mean of the sampling distribution of the mean. That is a mouthful alright! That is why we say the expected value of a statistic rather than the mean of a statistic. The latter would lead us to say ridiculous things like the mean of the variance, the mean of the standard deviation ... yuck! Anyway, its called the expected value because it's the average value over many samples. 
...
The test compares the value of the statistic to some null-hypothesized value of the population parameter. The null hypothesis is usually that the population parameter is equal to zero. 
...
When we conduct a t-test we are basically saying "if the sample statistic is close the null hypothesized value of the parameter, then  $t$ should be close to zero". The null hypothesis usually translates into "no effect", so if $t$ is close to zero, it means there was no effect. If $t$ is far away from zero, it means there was an effect. 

The problem in all of this is to give precise meaning to the terms "close" and "far". This is what we do when setting the significance level of the test, usually denoted $\alpha$. This number represents our tolerance for false positives or Type I errors -- i.e., for incorrectly concluding that the null hypothesis is true (no effect). When we set $\alpha$ to a small number, we are saying that we want the probability of a false positives to be small. This means we are going to need strong evidence (i.e., a large value of $t$) before we reject the null hypothesis. Still, the question is "how large?"

We denote the t-test using the letter $t$ because it has a t distribution when the null hypothesis is true (i.e., when the population parameter is equal to the null hypothesized value). So, this distribution tells what are the typical values of the t-test, if the null hypothesis is true. Some examples are shown below. 
...
Unlike the normal, the t distribution has a single parameter called its "degrees of freedom", which is denoted as $\nu$ in the figure. The degrees of freedom are always an increasing function of the sample size, so that larger samples lead to more degrees of freedom. When the degrees of freedom approach $\infty$, the t distribution approaches a normal distribution. When the degrees of freedom are small, the t distribution has wider tails than the normal distribution. This is important when doing statistical tests, because we are interested in values in the tails of the distribution -- i.e., large values of $t$. 

We use the t-test to make an inference about the null hypothesis by comparing two probabilities: 

1. The probability of observing a value of $t$ at least as large as the value in our sample, under the t distribution (i.e., under the assumption that the null hypothesis is true). This is called the $p$-value. 

2. The significance level that represents our tolerance for a false positive, $\alpha$.

If $p < \alpha$ (i.e., if $t$ is "far enough" away from zero) we reject the null hypothesis. Otherwise we retain the null hypothesis and conclude there was no effect.

There is a lot more to say here, but all of this is what Stat 1 was for. Usually we set $\alpha = .05$ and that is that. 

For an example of a t-test in R, see section \@ref(inference-for-slope-2).

### Confidence intervals 

A confidence interval is basically the same as a t-test, except we solve for the population parameter rather than the value of $t$. Whereas a t-test lets us make a guess about specific value of the parameter of interest (i.e., the null-hypothesized value), a confidence interval gives us a range of values that we can be sure include the parameter of interest, with some degree of "confidence." 
...
It is important that you can write up the results of an analysis in a way that other people will understand. For this reason, there are conventions about how to report statistical results. In this class, we will mainly use Table and Figures (formatted in R) rather than inline text. But sometimes the latter is inevitable, so you should know how to do it. 
